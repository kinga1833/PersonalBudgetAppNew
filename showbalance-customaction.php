...
<head>
	...
	<meta name="description" content="Zacznij panowac nad swoimi finansami!"/>
...
<body>
	<div class="container-fluid">
		<header>
			...
			<h3>Aplikacja do zarzadzania budzetem osobistym</h3>
	
			<nav class="navbar mt-4 bg-white navbar-expand-lg">
				
				<button class="navbar-toggler ml-2" type="button" data-toggle="collapse" data-target="#mainmenu" aria-controls="mainmenu" aria-expanded="false" aria-label="Przelacznik nawigacji">
					...
					<ul class="navbar-nav mx-auto bg-white">
						...
						<li class="col-lg-2 nav-item dropdown active">
							...
							<div class="dropdown-menu m-0 p-0" aria-labelledby="submenu">
								...
								.php">bieżacy miesiac</a>
								<a class="dropdown-item " href="showbalance-lastmonth.php">poprzedni miesiac</a>
								<a class="dropdown-item " href="showbalance-currentyear.php">bieżacy rok</a>
						...
						<li class="col-lg-2 mr-1 nav-item">
							<a class="nav-link" href="logout.php"><i class="icon-logout mr-2"></i>Wyloguj sie</a>
						</li>
					</ul>
				</div>
			</nav>
			...
			<section class="mx-auto px-2">
				<div class="row">
					<div class="addoperation text-center py-4 my-5 mx-auto bg-white col-md-10 col-lg-8">
						...
						<p>Wybierz przedzial czasowy</p>
					<div class="beginendperiod my-2 py-2">
							<form action="showbalance-customaction.php" method="post">
								Od
								<input type="date" name="startdate" class="ml-2 mr-4"/>
							
								Do
								<input type="date" name="enddate" class="ml-2 mr-4"/></br>
...
<?php
	...
	$beginOfPeriod = $_POST['startdate'];
	$endOfPeriod = $_POST['enddate'];
	$userid = $_SESSION['id'];	
	...
	{
		$_SESSION['fail'] = '<span style ="color:red">Wyświetlenie bilansu nie powiodło sie, spróbuj ponownie.</span>';
	...
	else
	{
		echo'<tr><td class="tablesubtitle">Data</td><td class="tablesubtitle">Kategoria</td><td class="tablesubtitle">Komentarz</td><td class="tablesubtitle">Kwota</td></tr>';
			...
			{
				echo '<tr><td>'.$incomeSQL['date_of_income']." PLN </td>";
...
<?php
	...
	if ($balance>=0) 
		echo "<span style='color:green'>Gratulacje. Świetnie zarzadzasz finansami!";
	else 
